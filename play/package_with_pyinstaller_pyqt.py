"""
Build script for PyInstaller packaging of MAME Controls with PyQt as default UI,
using hidden scripts and optimized directory structure
"""
import os
import subprocess
import shutil
...
def main():
    print("Starting PyInstaller packaging for MAME Controls with PyQt...")
    ...
    cmd = [
        ...
        "mame_controls_main_pyqt.py"  # Use the PyQt version of the main script
    ]
    ...
    print(f"  {dist_dir}/")
    print(f"  └── preview/")
    print(f"      ├── MAME_Controls{'.exe' if sys.platform == 'win32' else ''}")
    print(f"      ├── myData/")
    print(f"      ├── settings/")
    print(f"      └── [Python source files for import resolution]")
